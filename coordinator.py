...
class SidecarFallbackCoordinator:
    ...
    def build_source_plan(
        ...
        *,
        ...
        ready_indexes = {
            ...
            if snapshot.state is SidecarState.READY
        }
        unresolved_states = {
            SidecarState.QUEUED,
        ...
        waited_ms = 0.0
        final = initial
        if unresolved_indexes and self._near_ready_wait_ms > 0.0:
            wait_start = _now_ms()
        ...
        claim_by_index: dict[int, bool] = {}
        if claim_targets:
            claims = self._manager.try_fallback_claim(claim_targets, self._claimer_id)
            claim_by_index = {
                claim.handle.request_media_index: bool(claim.granted)
                for claim in claims
            }
        ...
        for descriptor, handle in zip(descriptors, handles):
            ...
            if claim_by_index.get(handle.request_media_index, False):
                entries.append(
                    SourcePlanEntry(
                        request_media_index=handle.request_media_index,
    ...
    def fetch_according_to_plan(
        ...
        *,
        ...
        plan = self.build_source_plan(descriptors=descriptors, handles=handles)
        descriptor_by_index = {
